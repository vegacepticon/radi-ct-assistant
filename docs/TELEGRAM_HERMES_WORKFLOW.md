...
| `РКТ описание находки` / `РКТ находка` | `finding_description` | Сформулировать только одну указанную находку |
| `РКТ описание + заключение` | `description_and_conclusion` | Сформировать оба блока |

Порядок важен: длинные инициаторы (`РКТ описание + заключение`, `РКТ описание находки`)
распознаются раньше короткого `РКТ описание`.

### Одна находка и уточняющие вопросы

Для просьбы сформулировать одну находку используйте `finding_description`, а не
общий `description`. Этот task:

- возвращает только формулировку указанной находки, не полный протокол;
- принудительно использует `output_mode=findings_only`;
- не добавляет нормальные структуры, заключение или рекомендации;
- при клинически значимой неоднозначности возвращает только блок
  `Уточняющие вопросы:` — не более трёх вопросов.

Блок уточняющих вопросов является промежуточным диалоговым ответом. API отклоняет
его при `/api/draft` и `/api/save-draft` с HTTP 409. После ответа пользователя
Hermes дополняет исходный ввод и повторяет `prepare`; backend не хранит отдельный
многошаговый статус вопросов.
...
Область: ОГК
---
В S8 справа солидный очаг 15 мм с ровными контурами.
```
...
| `Формат вывода` | `full_systematic` / `findings_only` | полный систематический текст или только переданные находки; для `finding_description` всегда `findings_only` |
